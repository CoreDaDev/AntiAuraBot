...
class EventListener implements Listener {
    ...
    public function onDamage(EntityDamageEvent $event) {
        ...
        if((in_array($damage->getName(), AntiAuraBot::$reports) && $this->aps[$damage->getName()][0] > 4) || (AntiAuraBot::$config->getNested("auto-report.enabled") && $this->aps[$damage->getName()][0] > AntiAuraBot::$config->getNested("auto-report.warns"))) {
            self::$report[$damage->getName()] = true;
            if(in_array($damage->getName(), AntiAuraBot::$reports))
                unset(AntiAuraBot::$reports[array_search($damage->getName(), AntiAuraBot::$reports)]);
            ...
            $bot = Entity::createEntity("AntiAuraBotEntity", $damage->level, $nbt);
