...
class BotEntity extends Human {
    ...
    public function onUpdate(int $currentTick): bool {
        $this->fullTicks++;
        if(!$this->hacker instanceof Player || $this->hacker->isClosed() || !$this->level || $this->fullTicks >= AntiAuraBot::$config->getNested("de"."spawn-seconds")*20) {
            $this->flagForDespawn();
            return false;
        }
        switch(AntiAuraBot::$config->getNested("teleport.type")) {
            case "turn":
                $this->teleport(
                    $this->hacker->add(
                        $this->getCustomVector()->multiply(
                            -AntiAuraBot::$config->getNested("teleport.back-multiplier")[array_rand(AntiAuraBot::$config->getNested("teleport.back-multiplier"))]
                        )
                    )->add(
                        0, AntiAuraBot::$config->getNested("teleport.turn-y")[array_rand(AntiAuraBot::$config->getNested("teleport.turn-y"))]
                    ),
                    $this->yaw,
                    $this->pitch
                );
                $this->yaw+=AntiAuraBot::$config->getNested("teleport.turn-speed")[array_rand(AntiAuraBot::$config->getNested("teleport.turn-speed"))];
                if($this->yaw > 360)
                    $this->yaw-=360;
                break;
            case "fast-teleport":
            case "fastTeleport":
                if($this->teleportTick >= AntiAuraBot::$config->getNested("teleport.teleport-speed")) {
                    $expand = AntiAuraBot::$config->getNested("teleport.teleport-range");
            ...
            case "backSide":
                $this->lookAt($this->hacker);
                $this->teleport($this->hacker->add($this->getCustomHackerVector()->multiply(-AntiAuraBot::$config->getNested("teleport.back-multiplier")[array_rand(AntiAuraBot::$config->getNested("teleport.back-multiplier"))])));
                break;
        }
        return true;
    }
    ...
    public function attack(EntityDamageEvent $source): void {
        if($source instanceof EntityDamageByEntityEvent && $source->getDamager()->id == $this->hacker->id)
            $this->warns++;
        if($this->hacker instanceof Player && $this->hacker->isOnline() && $this->warns >= AntiAuraBot::$config->getNested("warns"))
            switch(AntiAuraBot::$config->getNested("punishment")) {
                case "kick":
                    $this->hacker->kick(AntiAuraBot::$config->getNested("reason"), false);
                    break;
                case "ban":
                    $this->server->getNameBans()->addBan($this->getName(), AntiAuraBot::$config->getNested("reason"), null, "AntiAuraBot AC");
                    $this->hacker->kick(AntiAuraBot::$config->getNested("reason"), false);
                    break;
                default:
                    $cmd = AntiAuraBot::$config->getNested("punishment");
                    if(is_array($cmd))
                        foreach($cmd as $c) {
