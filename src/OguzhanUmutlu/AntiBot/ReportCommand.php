...
class ReportCommand extends Command {
    public function __construct() {
        parent::__construct(AntiBot::$config->getNested("report-command.name"), AntiBot::$config->getNested("report-command.description"), null, []);
    }
    ...
    public function execute(CommandSender $sender, string $commandLabel, array $args) {
        ...
        if(isset(AntiBot::$reports[$sender->getName()])) {
            $sender->sendMessage("§c> You already reported someone!");
            return;
        }
        if(!in_array($player->getName(), AntiBot::$reports))
            AntiBot::$reports[$sender->getName()] = $player->getName();
